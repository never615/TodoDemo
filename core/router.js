...
function resolve(routes, context) {
  for (const route of routes) {
    ...
    if (route.data) {
      ...
      const keys = Object.keys(route.data);   //object扩展keys
      return Promise.all([ //promise对象
        route.load(),
        ...keys.map(key => {  //扩展运算符
      ...
      ]).then(([Page, ...data]) => {  //rest参数
        const props = keys.reduce((result, key, i) => ({...result, [key]: data[i]}), {});   //解构赋值/箭头函数/属性名表达式/扩展运算符
